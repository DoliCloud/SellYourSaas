...
/**
 *   	\file       whitelistemail_list.php
 *		\ingroup    sellyoursaas
 *		\brief      List page for whitelistemail
 */
...
require_once __DIR__.'/class/whitelistemail.class.php';
...
$hookmanager->initHooks(array('whitelistemaillist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->sellyoursaas->whitelistemail->write;
	$permissiontodelete = $user->rights->sellyoursaas->whitelistemail->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Whitelistemail';
	$objectlabel = 'Whitelistemail';
...
//$help_url = "EN:Module_Whitelistemail|FR:Module_Whitelistemail_FR|ES:Módulo_Whitelistemail";
$help_url = '';
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/sellyoursaas/whitelistemail_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/sellyoursaas/whitelistemail_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

print '<span class="opacitymedium">'.$langs->trans("WhitelistemailDesc").'</span><br><br>';

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendWhitelistmailRef";
$modelmail = "whitelistemail";
$objecttmp = new Whitelistemail($db);
