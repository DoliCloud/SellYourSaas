...
* Obtenir un ou n serveurs avec un accès SSH pouvant passer root (On utilisera Ubuntu LTS minimal 16.04 ou 18.04) pour le serveur de Déploiement. Remarque: Cette étape peut être ignorée si vous décidez que le serveur de Déploiement sera le même serveur que le serveur Master.
...
* Installation et activation du watchdog apache dans */home/admin/wwwroot/dolibarr_sellyoursaas/scripts/remote_server_launcher.sh* en créant un lien par
...
Donner les droits d'accès sur le serveur de déploiement pour administration à distance:

[source,sql]
---------------
CREATE USER 'yourlogin'@'ip.poste.admin.distant' IDENTIFIED BY '...';
GRANT CREATE USER,GRANT OPTION,RELOAD ON *.* TO 'sellyoursaas'@'ipserverdeployment';
GRANT CREATE,CREATE TEMPORARY TABLES,CREATE VIEW,DROP,DELETE,INSERT,SELECT,UPDATE,ALTER,INDEX,LOCK TABLES,REFERENCES,SHOW VIEW ON *.* TO 'sellyoursaas'@'ipserverdeployment';
FLUSH PRIVILEGES;
---------------
...
Si vous utilisez la supervision DataDog, créer un compte datadog pouvant accéder locallement à la base:
...
- Créer un nouveau serveur OVH depuis le snapshot en spécifiant le nom du snapshot source et en indiquant le script suivant en *post installation*:
    ...
    /home/admin/wwwroot/dolibarr_sellyoursaas/scripts/post_inst_script.sh
...
[source,bash]
---------------
${IPTABLES} -t filter -A INPUT -i ens3 -p tcp -s ip.server.deploymeent --dport nfs -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
${IPTABLES} -t filter -A INPUT -i ens3 -p udp -s ip.server.deploymeent --dport nfs -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
${IPTABLES} -t filter -A OUTPUT -o ens3 -p tcp -d ip.server.deploymeent --sport nfs -m state --state ESTABLISHED,RELATED -j ACCEPT
${IPTABLES} -t filter -A OUTPUT -o ens3 -p udp -d ip.server.deploymeent --sport nfs -m state --state ESTABLISHED,RELATED -j ACCEPT
---------------
...
- Ajouter les entrées du nouveau serveur dans le DNS de *mysellyoursaasdomainname.com* avec l'IP v4 (record A) et l'IP v6 (record AAA) du serveur attribué par OVH lors de sa création.

- Aller dans l'interface de gestion des serveurs OVH, pour modifier leur nom court. Ceci modifiera le fichier */etc/hostname* automatiquement (sinon modifier manuellement. Mettre un nom court, par exemple *myserver*).

- Aller dans l'interface de gestion des IP OVH, pour ajouter aussi 
 
  - le reverse sur l'IP du nouveau serveur: *myserver.mysellyoursaasdomainname.com*
