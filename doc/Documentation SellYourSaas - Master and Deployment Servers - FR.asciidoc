...
On va ajouter un disque indépendant pour les instances utilisateurs et sauvegardes. Il peut s'agir d'un disque pour les 2 ou bien de 2 disques différents.
...
Pour cela, créer un fichier */etc/fail2ban/jail.local* avec ce contenu:

NOTE: Les règles disponibles peuvent varier selon la version de l'OS installé.

NOTE: Penser à modifier aussi "mybusinessips" par votre ou vos ips séparés par des espaces ainsi que la valeur de destemail par l'email de supervision de votre société.
