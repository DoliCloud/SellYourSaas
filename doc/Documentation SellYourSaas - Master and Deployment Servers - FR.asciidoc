...
Pour éviter que apparmor se relance en cas d'arrêt manuel, modifier */lib/systemd/system/apparmor.service* pour décommenter *RemainAfterExit=no*
[source,bash]
---------------
RemainAfterExit=no
---------------
...
NOTE: Si le lancement est long, pour suivre l'état d'avancement, faire un lsof -p PID_de_mysql pour voir les fichiers et base traitées en relog. Ils sont ouvertes par ordre de dates sur le disque.
