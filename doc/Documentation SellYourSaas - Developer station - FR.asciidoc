...
- Start the computer with the USB key as boot
- Choose to start CloneZilla and choose the SSH for the media storing the image.
- Choose saveparts (or savedisks)

Restore

- First go on source image. Rename the image file sdXY.xxx-ptcl-img.gz to change XY whith value to use to restore to.
- Edit file *dev-fs.list*

- Launch CloneZilla then restoreparts
