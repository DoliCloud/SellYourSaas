...
This document explains how to install the optional web portal server of Sell-Your-Saas.
...
#doldataroot=/home/admin/wwwroot/dolibarr_documents
...
* DKIM to sign some information in email.
...
* Edit file */etc/systemd/journald.conf* to define the max size of file journal systemd
...
=== Install certbot (for certificates SSL)
...
Reprendre du fichier */etc/crontab*, les commandes pour tester le lancement de crontab journalière, hebdo et mensuelles et tester en lançant en manuel. For example with:
...
Edit it to follow this processes:
...
== Installation of components Backups/Synchro
...
Note that once restoration is done, if ir is done an another server, you must:
