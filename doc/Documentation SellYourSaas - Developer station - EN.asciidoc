...
Replace the "stringtoshow" with your sentence to output.
...
=== Add json or xml formatter to your text editor like Geany

sudo apt update && sudo apt install geany-plugins

Then in Geany, Edit - Format - Send to.

Ajouter la commande "jq" pour le formatage de JSON.
Ajouter la commande "xmllint --format -"  pour le formatage de XML.
...
=== Error on push/pull from Eclipse IDE with Ubuntu 26.04

Add this into the .ssh/config to force the protocol to use for SSH. Ubuntu 26.04 use a more secured one with "publickey-hostbound@openssh.com" not supported by Egit.
...
Host github.com
    ...
    IdentitiesOnly yes
    PubkeyAcceptedAlgorithms ssh-rsa,ecdsa-sha2-nistp256,ecdsa-sha2-nistp384,ecdsa-sha2-nistp521,sk-ssh-ed25519@openssh.com
    HostKeyAlgorithms ssh-rsa,ecdsa-sha2-nistp256,ecdsa-sha2-nistp384,ecdsa-sha2-nistp521,sk-ssh-ed25519@openssh.com
    KexAlgorithms diffie-hellman-group-exchange-sha256,diffie-hellman-group14-sha1
    StrictHostKeyChecking yes
---------------

Note: The commande *ssh-agent -s* must return something like this:

[source,bash]
---------------
SSH_AUTH_SOCK=/home/mylogin/.ssh/agent/s.419v2MPStb.agent.7dWwsVSLUf; export SSH_AUTH_SOCK;
SSH_AGENT_PID=461316; export SSH_AGENT_PID;
echo Agent pid 461316;
---------------

You can force the addition of the key with :  ssh-add ~/.ssh/id_rsa or ssh-add ~/.ssh/id_ed25519
