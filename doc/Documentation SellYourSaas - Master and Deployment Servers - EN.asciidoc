...
Create a file *myunixlogin* to put in the directory */etc/sudoers.d* with the owner *root*.*root* and the permissions *r-r-----* and the content
...
Do not forget to set a value for *databasepass*.
...
Increase the timeout for launching processes because sometimes mysql / mariadb can take a long time to restatr after a crash. To do this, modify the file */etc/systemd/system/mariadb.service.d/migrated-from-my.cnf-settings.conf* and put
...
[source,sql]
---------------
CREATE USER 'yourremotelogin'@'ip.poste.admin.distant' IDENTIFIED BY '...passwordfordatadog...';
...
We have to create the DNS files that will be used to store the DNS record of each customer. Those files will be completed after each new deployment.
...
=== Clone an server instance for a another production server or for development
