...
And if a disk was created for the backup (ignore the 2 next instruction if not):
...
Rem: If you need to recover data files from another source disk, use:
...
Into the following instructions, we will use X with value '', 2, 3, ... (digit of pool of instance = digit of the deployment server).
