...
[source,bash]
---------------
inet_interfaces = public_ip_returned_by_server_s_reverse_dns
inet_protocols = ipv4
---------------
...
In the registrar, update the DNS of the main domain *mysaasdomainname.com* with *A* records 
    ...
    *ns1with1.mysaasdomainname.com*       DNS server 1 of the user instances, points to the deployment server's IP
    *ns2with1.mysaasdomainname.com*       DNS server 2 of the user instances, points to the deployment server's IP
...
[source,bash]
---------------
mkdir -p /var/lib/sellyoursaas/; chmod a+rwx /var/lib/sellyoursaas/
---------------

And link files on master or copy them on deployment server:

[source,bash]
---------------
On master server:
ln -fs /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistmail /var/lib/sellyoursaas/blacklistmail;
ln -fs /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistip /var/lib/sellyoursaas/blacklistip;
ln -fs /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistfrom /var/lib/sellyoursaas/blacklistfrom;
ln -fs /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistcontent /var/lib/sellyoursaas/blacklistcontent;
On deployment server:
cp -p /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistmail /var/lib/sellyoursaas/blacklistmail;
cp -p /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistip /var/lib/sellyoursaas/blacklistip;
cp -p /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistfrom /var/lib/sellyoursaas/blacklistfrom;
cp -p /home/admin/wwwroot/dolibarr_documents/sellyoursaas/spam/blacklistcontent /var/lib/sellyoursaas/blacklistcontent;
chmod -R a+rw /var/lib/sellyoursaas;
---------------
...
* Activate the module SellYourSaas

* Create a generic user account Dolibarr to access the client interface (for instance: "Client user"). Give him the permissions below:
...
* Create a *Service* of *Application* type to define the pricing of a subscription and associated *Package*.
...
- Configuration of the server and paying instances is done locally via the cron
...
- Paying user instances are backed up locally by the cron
...
For example, to deploy an application like Dolibarr, with sources in directory *documents/sellyoursaas/git*, enter the values of the following fields (Note: variables __XXX__ are reserved keywrods that will be replaced by SellYourSaas process that uses those fields):
...
- Copy/create/update configuration files and update them with all variables.
...
The client (maybe an old PHP version) uses a deprecated algorithm for negotiation. Edit the sshd_config of the server by adding:

# Legacy changes - To allow an old client to connect to
KexAlgorithms +diffie-hellman-group1-sha1
Ciphers +aes128-cbc

Then reload ssh with /etc/init.d/ssh reload
