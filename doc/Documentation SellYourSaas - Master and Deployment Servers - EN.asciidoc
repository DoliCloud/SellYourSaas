...
#Uncomment those lignes to use Sendgrid as a mailserver for emails coming from user instances
...
# Ou mettre relayhost à vide pour utiliser le serveur local comme agent d'envoi des emails.
...
* Create a generic Dolibarr user account to access the client interface (for instance: "Client user"). Give him the permissions below
and only the permission below:
...
- Aquire the SSL certificates of the new subdomain *withX.mydomain.com*, upload them to the server and create the symbolic links. (see <<ssl_certificate_creation>>).
...
(Avec cette valeur, il sera alors possible de soumettre le fichier dump à charger via l'onglet *Fichiers joints*
