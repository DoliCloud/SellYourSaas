...
; enabled = true for master, false for deployment
enabled = true
port    = http,https
...
For deployment server only, disable into */etc/fail2ban/jail.local* the 3 rules starting with web-dolibarr-* (they must be used on Master server only)
