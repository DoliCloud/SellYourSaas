...
* Run cerbot with the following command line *on the master* :
...
On the *deployment servers*, to get automatically renewable wildcard certificates :
...
* To renew a certificate *on the master*, run
...
For wildcard certificates on *deployment servers*, you can use the same command as used originally (with --dry-run option first if you want to check)
...
And to check that a DNS update is ok for a challenge, you can use this URL: https://toolbox.googleapps.com/apps/dig/#TXT/_acme-challenge.withX.mysaasdomainname.com
