...
The client (maybe an old PHP version) uses a deprecated algorithm for negotiation. Edit the file */etc/ssh/sshd_config.d/sellyousaas.conf* of the server by adding:
...
To use a different version of lib for PHP to make ssh2_connect, you can run phpize, Recompile libssh2 and Recompile php-ssh2:

[source,bash]
---------------
git clone https://github.com/libssh2/libssh2.git libssh2; ./configure; make; make install
