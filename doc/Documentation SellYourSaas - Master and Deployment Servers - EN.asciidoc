...
[source,bash]
---------------
passthru,shell_exec,system,proc_open,popen,dl,apache_note,apache_setenv,show_source,virtual
---------------
...
Create a server from the image. If after creating the server, you can't connect to SSH, may be problem is the one described on the trouble shooting "sshd: no hostkeys available -- exiting"

Detach IP on the 2 servers. Then attach IP of the production serveur to the new one. 

Go on the new server using the new IP to set the hostname with same value than old name:
...
=== Booting a server in rescue mode / Restoring a root access or a server blocked by a firewall
...
or the root password for the resuce mode will be shown).
...
=== Analysing changes in system files

This can be done using afick.pl
...
It may mean that you IPv6 connection is wrong. Disable the firewall, and try to disable the IPv6 interface
