...
=== Booting a server in rescue mode / Restoring the root access
...
To restore a root access you can:
...
=== Reset root password with boot rescue

First you need to reboot your server in boot rescue mode.

==== To reboot your server in boot rescue
With OVH Public Cloud :

* Click on the instance where you want  to reset the password. 

* Click on the "Reboot in rescue mode" entry and continue by using the button "Restart" in the popup.

* Once done go in the menu VNC console ( it will open a console directly connected to the server).

* When the server is completely restarted a login and a password will be displayed, use it to connect to the server.

* After connecting, you need to identify the system partition. This can be done with the following command:
...
In this exemple, the system partition is /dev/hda1.

  Note : if your server is in RAID configuration the system partition is the raid volume partition.
