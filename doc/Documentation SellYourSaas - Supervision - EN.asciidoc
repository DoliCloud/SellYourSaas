= Sell-Your-Saas installation and operation document - Supervision platform
This document describes the technical and functional implementation of the supervision platform
:source-highlighter: red
:title: Document installation and operation for the supervision platform
:subject: This document describes the technical and functional implementation of SellYourSaas supervision platform
:keywords: sellyoursaas, saas, supervision, wazuh
:toc: manual
:toclevels: 3
...
This document explains how to install the optional supervision platform of Sell-Your-Saas.


== Installation of the machine and OS of the supervision platform

=== Choice of the machine and OS and create it ===

A 2 core server with 8Gb should be enough.
...
Run *ssh-keygen* for all these accounts : *root*, *admin* and *myunixlogin*
...
* DKIM permet de signer certaines information du mail.
...
=== Configuration de Postfix pour authentication externe SMTP sécurisée (optionnel)
...
Check that postfix user is inside group *sasl*. If not, add it with:
...
* Edit file */etc/systemd/journald.conf* to define the ma size of journal systemd
...
== Installation composant Backups/Synchro
...
Note that once a restoration has been done, if done from another server, you must:
