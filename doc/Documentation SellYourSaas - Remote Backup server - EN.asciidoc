...
55 23 * * * btrfs subvolume delete /mnt/diskbackup/.snapshots/diskbackup-$(date +%j); btrfs subvolume snapshot /mnt/diskbackup /mnt/diskbackup/.snapshots/diskbackup-$(date +%j); rm -fr /mnt/diskbackup/.snapshots/diskbackup-$(date +%j)/.snapshots; btrfs property set -ts /mnt/diskbackup/.snapshots/diskbackup-$(date +%j) ro true;
---------------
...
=== Use of btrfs to create a subvolume for each backup dir of each server (optional)
...
To convert an existing directory into a subvolume

[source,bash]
---------------
su - admin

mv /mnt/diskbackup/xxx /mnt/diskbackup/xxx-old
btrfs subvolume create /mnt/diskbackup/xxx   # with xxx = home_serverX or backup_serverX
cp -ax --reflink=always /mnt/diskbackup/xxx-old/. /mnt/diskbackup/xxx
rm -fr /mnt/diskbackup/xxx-old
...
-- Example of CLI to move not dir into btrfs subvolume
