...
* Declare the mount for an automatic mount at each reboot by adding a line in */etc/fstab*
...
* Mount disk manually the first time
...
* Optimize filesystem it by removing the update of the "atime" read access

To see options for optimizing filesystems:

[source, bash]
---------------
cat /proc/mounts | grep time
---------------
...
Only */mnt/diskbackup*, already created previously, is required, so no creation of directory has to be done.
