...
/**
 *   	\file       deploymentserver_list.php
 *		\ingroup    sellyoursaas
 *		\brief      List page for deploymentserver
 */
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('sellyoursaas', 'deploymentserver', 'read');
	$permissiontoadd = $user->hasRight('sellyoursaas', 'deploymentserver', 'write');
	$permissiontodelete = $user->hasRight('sellyoursaas', 'deploymentserver', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("sellyoursaas")) {
	accessforbidden('Module sellyoursaas not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Deploymentserver';
	$objectlabel = 'Deploymentserver';
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/sellyoursaas/deploymentserver_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDeploymentserverRef";
$modelmail = "deploymentserver";
$objecttmp = new Deploymentserver($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if DEPLOYMENTSERVER_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
	...
	if ($mode == 'kanban') {
		...
		if (!empty($arrayfields['nb_backups']['checked'])) {
			...
			if (!empty($backuptotalinstances[$obj->ipaddress])) {
				print '<a href="'.DOL_URL_ROOT.'/contrat/list.php?search_options_deployment_status[]=done&search_options_deployment_status[]=processing&search_options_latestbackup_status='.urlencode('KO|OK').'&search_options_deployment_host='.urlencode($obj->ipaddress).'&search_options_suspendmaintenance_message=%5E%24">';
