...
/**
 *   	\file       blacklistmail_list.php
 *		\ingroup    sellyoursaas
 *		\brief      List page for blacklistmail
 */
...
require_once __DIR__.'/class/blacklistmail.class.php';
...
$hookmanager->initHooks(array('blacklistmaillist')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Blacklistmail';
	$objectlabel = 'Blacklistmail';
...
$help_url = '';
$title = $langs->trans("Blacklistmails");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/sellyoursaas/blacklistmail_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/sellyoursaas/blacklistmail_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$objecttmp = new Blacklistmail($db);
