...
/**
 *   	\file       blacklistfrom_list.php
 *		\ingroup    sellyoursaas
 *		\brief      List page for blacklistfrom
 */
...
require_once __DIR__.'/class/blacklistfrom.class.php';
...
$hookmanager->initHooks(array('blacklistfromlist')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Blacklistfrom';
	$objectlabel = 'Blacklistfrom';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/sellyoursaas/blacklistfrom_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/sellyoursaas/blacklistfrom_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$objecttmp = new Blacklistfrom($db);
