...
/**
 *   	\file       deploymentserver_card.php
 *		\ingroup    sellyoursaas
 *		\brief      Page to create/edit/view deploymentserver
 */
...
$hookmanager->initHooks(array('deploymentservercard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->sellyoursaas->deploymentserver->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->rights->sellyoursaas->deploymentserver->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->rights->sellyoursaas->deploymentserver->write; // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->rights->sellyoursaas->deploymentserver->write; // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->sellyoursaas->multidir_output[isset($object->entity) ? $object->entity : 1].'/deploymentserver';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/sellyoursaas/deploymentserver_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/sellyoursaas/deploymentserver_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'SELLYOURSAAS_DEPLOYMENTSERVER_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'SELLYOURSAAS_DEPLOYMENTSERVER_SENTBYMAIL';
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Deploymentserver")), '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Deploymentserver"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Deploymentserver"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteDeploymentserver'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionDeploymentserver', $object->ref);
		/*if (isModEnabled('notification'))
		{
			require_once DOL_DOCUMENT_ROOT . '/core/class/notify.class.php';
			$notify = new Notify($db);
			$text .= '<br>';
			$text .= $notify->confirmMessage('DEPLOYMENTSERVER_CLOSE', $object->socid, $object);
		}*/
	...
	/*if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('sellyoursaas:Deploymentserver', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/sellyoursaas/deploymentserver_agenda.php', 1).'?id='.$object->id);
