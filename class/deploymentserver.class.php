...
/**
 * \file        class/deploymentserver.class.php
 * \ingroup     sellyoursaas
 * \brief       This file is a CRUD class file for Deploymentserver (Create/Read/Update/Delete)
 */
...
/**
 * Class for Deploymentserver
 */
class Deploymentserver extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'deploymentserver';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'sellyoursaas_deploymentserver';
	...
	/**
	 * @var string String with name of icon for deploymentserver. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'deploymentserver@sellyoursaas' if picto is file 'img/object_deploymentserver.png'.
	 */
	...
	public $fields=array(
		...
		'status' => array('type'=>'integer', 'label'=>'Status', 'enabled'=>'1', 'position'=>52, 'notnull'=>1, 'visible'=>1, 'index'=>1, 'arrayofkeyval'=>array('0'=>'Désactivé', '1'=>'Actif'), 'validate'=>'1',),
		...
		'domainname' => array('type'=>'varchar(128)', 'label'=>'Domain', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>1,),
		'subdomainname' => array('type'=>'varchar(128)', 'label'=>'SubDomainName', 'enabled'=>'1', 'position'=>56, 'notnull'=>0, 'visible'=>1, 'help'=>"SubDomainNameInfo",),
		'ipaddress' => array('type'=>'varchar(128)', 'label'=>'IP', 'enabled'=>'1', 'position'=>51, 'notnull'=>1, 'visible'=>1,),
		'servercustomerannouncestatus' => array('type'=>'integer', 'label'=>'ServerCustomerAnnounceStatus', 'enabled'=>'1', 'position'=>61, 'notnull'=>0, 'visible'=>1, 'arrayofkeyval'=>array('0'=>'D&eacute;sactiv&eacute;', '1'=>'Actif'),),
		'servercustomerannounce' => array('type'=>'text', 'label'=>'ServerCustomerAnnounce', 'enabled'=>'1', 'position'=>62, 'notnull'=>0, 'visible'=>1,),
	);
	public $rowid;
	public $note_private;
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->sellyoursaas->deploymentserver->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->sellyoursaas->deploymentserver->write))
		 || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->sellyoursaas->deploymentserver->deploymentserver_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			$sql .= " status = ".self::STATUS_ENABLED;
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('DEPLOYMENTSERVER_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'deploymentserver/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'deploymentserver/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->sellyoursaas->dir_output.'/deploymentserver/'.$oldref;
				$dirdest = $conf->sellyoursaas->dir_output.'/deploymentserver/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->sellyoursaas->dir_output.'/deploymentserver/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DISABLED, $notrigger, 'DEPLOYMENTSERVER_UNVALIDATE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_ENABLED, $notrigger, 'DEPLOYMENTSERVER_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$hookmanager->initHooks(array('deploymentserverdao'));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_deploymentserver';
