		...
		if ($object->element == 'societe') {
			...
			if ($user->rights->sellyoursaas->read && ! empty($object->array_options['options_dolicloud'])) {
				...
				if ($object->array_options['options_dolicloud'] == 'yesv2') {
					...
					$url=$urlmyaccount.'?mode=logout_dashboard&action=login&token='.newToken().'&actionlogin=login&username='.urlencode($object->email).'&password=&login_hash='.$dol_login_hash;
				}
