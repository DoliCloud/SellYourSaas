...
function getListOfLinks($object, $lastloginadmin, $lastpassadmin)
{
	...
	if (!empty($object->array_options['options_custom_url'])) {
		// doldataroot is only known on the target deployment server (its own /etc/sellyoursaas.conf),
		...
		$generatecertif ='doldataroot=`grep "^doldataroot=" /etc/sellyoursaas.conf | cut -d "=" -f 2`'."\n";
		$generatecertif.='certbot certonly --webroot -w '.$homestring.'/'.$object->database_db.'/htdocs -d '.$object->array_options['options_custom_url']."\n";
		$generatecertif.='ln -fs /etc/letsencrypt/live/'.$object->array_options['options_custom_url'].'/privkey.pem ${doldataroot:-/home/admin/wwwroot/dolibarr_documents}/sellyoursaas_local/crt/'.$object->ref_customer.'-custom.key'."\n";
		$generatecertif.='ln -fs /etc/letsencrypt/live/'.$object->array_options['options_custom_url'].'/cert.pem ${doldataroot:-/home/admin/wwwroot/dolibarr_documents}/sellyoursaas_local/crt/'.$object->ref_customer.'-custom.crt'."\n";
		$generatecertif.='ln -fs /etc/letsencrypt/live/'.$object->array_options['options_custom_url'].'/fullchain.pem ${doldataroot:-/home/admin/wwwroot/dolibarr_documents}/sellyoursaas_local/crt/'.$object->ref_customer.'-custom-intermediate.crt'."\n";
	} else {
