...
if [[ $testorconfirm == "test" ]]; then
	...
	if [[ "x$idlistofdb" != "x" ]]; then
		...
		echo "INSERT INTO llx_contracttoupdate_tmp(nom, client, rowid, ref, ref_customer, deployment_date_start, undeployment_date, deployment_ip) SELECT s.nom, s.client, c.rowid, c.ref, c.ref_customer, ce.deployment_date_start, ce.undeployment_date FROM llx_contrat as c LEFT JOIN llx_societe as s ON s.rowid = c.fk_soc, llx_contrat_extrafields as ce WHERE c.rowid = ce.fk_object AND ce.database_db IN ($idlistofdb) AND ce.deployment_status = 'undeployed';" | $MYSQL -usellyoursaas -p$databasepass -h $databasehost $database
		echo
