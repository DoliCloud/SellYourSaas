...
echo $now" Try to detect lines 'wrote report /var/crash/_usr_sbin_apache2.0.crash' into $WDLOGFILE" >> /var/log/apache_watchdog.log 
...
while read ; do
	...
	echo "Apache seg fault detected by apache_watchdog_daemon2. Apache was killed and started." | mail -aFrom:$EMAILFROM -s "[Alert] Apache seg fault detected on "`hostname`". Apache was killed and started." $EMAILTO
	sleep 5
	export now=`date '+%Y%m%d%H%M%S'`
