...
file_put_contents($logfile, date('Y-m-d H:i:s') . " SERVER_NAME = ".(empty($_ENV['SERVER_NAME']) ? '' : $_ENV['SERVER_NAME'])."\n", FILE_APPEND);
file_put_contents($logfile, date('Y-m-d H:i:s') . " DOCUMENT_ROOT = ".(empty($_ENV['DOCUMENT_ROOT']) ? '' : $_ENV['DOCUMENT_ROOT'])."\n", FILE_APPEND);
...
file_put_contents($logfile, date('Y-m-d H:i:s')." nb of process found with ".$commandcheck." = ".$resexec." (we accept ".$MAXPERDAY.")", FILE_APPEND);
