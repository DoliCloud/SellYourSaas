...
/**
 *      \file       sellyoursaas/scripts/deployment_batch_detect_evil_instances.php
...
file_put_contents($pathtospamdir.'/mailquota', "# Content is generated by deployment_batch_detect_evil_instances.php from value of instance. If empty, value used is taken from /etc/sellyoursaas-public.conf (param maxemailperday and maxemailperdaypaid).\n", FILE_APPEND);

$i = 0;
foreach ($instances as $instanceid => $instancearray) {
	$i++;
	// We complete the file $filetobuild = $pathtospamdir.'/mailquota';
	echo 'Process paid/confirmed instance id='.$instancearray['id'].' ref='.$instancearray['ref'].' osu='.$instancearray['osu']." mailquota=".($instancearray['maxperday'] ? $instancearray['maxperday'] : $MAXPERDAYPAID)."\n";
