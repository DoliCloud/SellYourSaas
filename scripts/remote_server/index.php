...
function checkScriptFile($scriptfile, $fh, $params)
{
	...
	foreach ($rows as $line) {
		...
		if (preg_match('/^#?cd __INSTANCEDIR__\/htdocs\/install; php upgrade2.php 0\.0\.0 0\.0\.0 MAIN_MODULE_[a-z]+;$/i', $newline)) {
			continue;
		}
		// Check more patterns
		...
		if (preg_match('/^#?cd \/home\/jail\/home\/osu[a-z0-9]+\/dbn[a-z0-9]+\/htdocs\/install; php upgrade2.php 0\.0\.0 0\.0\.0 MAIN_MODULE_[a-z]+$/i', $newline)) {
			continue;
		}
