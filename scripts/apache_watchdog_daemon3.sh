...
while read ; do
	...
	echo "Apache seg fault detected by apache_watchdog_daemon3. Apache was killed and started." | mail -aFrom:$EMAILFROM -s "[Warning] Apache seg fault detected on "`hostname`". Apache was killed and started." $EMAILTO
	sleep 5
	export now=`date '+%Y%m%d%H%M%S'`
