...
function print_user_table($newdb, $object)
{
	...
	if (is_object($newdb) && $newdb->connected) {
		...
		if (preg_match('/glpi-network\.cloud/', $object->ref_customer)) {
			...
			$sql = "SELECT gu.id as rowid, gu.name as login, gu.realname as lastname, gu.firstname, gp.interface as admin, '' as pass, gu.password as pass_crypted, gu.date_creation as datec, gu.date_mod as datem, gu.last_login as datelastlogin, 0, 0, 0, gu.entities_id as entity, gu.is_active as statut,";
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				print '<td class="tdoverflowmax100">'.($obj->pass ? $obj->pass.' (' : '').($obj->pass_crypted?$obj->pass_crypted:'NA').($obj->pass ? ')' : '').'</td>';
