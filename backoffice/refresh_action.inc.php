...
if ($action == 'addauthorizedkey') {
	...
	if ($connection) {
		...
		if (! @ssh2_auth_password($connection, $username_web, $password_web)) {
			...
			$dir=preg_replace('/_([a-zA-Z0-9]+)$/', '', $database_db);
			//$fileinstallmoduleslock="ssh2.sftp://".$sftp.$conf->global->DOLICLOUD_INSTANCES_PATH.'/'.$username_web.'/'.$dir.'/documents/installmodules.lock';
			$fileinstallmoduleslock="ssh2.sftp://".intval($sftp).$conf->global->DOLICLOUD_INSTANCES_PATH.'/'.$username_web.'/'.$dir.'/documents/installmodules.lock';
	...
	if ($connection) {
		...
		if (! @ssh2_auth_password($connection, $username_web, $password_web)) {
			...
			// Check if installmodules.lock exists
			$dir=preg_replace('/_([a-zA-Z0-9]+)$/', '', $database_db);
			$filetodelete=$conf->global->DOLICLOUD_INSTANCES_PATH.'/'.$username_web.'/'.$dir.'/documents/installmodules.lock';
			$result=ssh2_sftp_unlink($sftp, $filetodelete);

			if ($result) setEventMessage($langs->transnoentitiesnoconv("FileDeleted"), 'mesgs');
			else setEventMessage($langs->transnoentitiesnoconv("ErrorFailToDeleteFile", $username_web.'/'.$dir.'/documents/installmodules.lock'), 'warnings');
