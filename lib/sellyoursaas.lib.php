...
function sellyoursaasGetExpirationDate($contract)
{
	...
	foreach($contract->lines as $line)
	{
		...
		if ($line->fk_product > 0)
		{
			...
			{
			    if (preg_match('/user/i', $prodforline->array_options['options_resource_label'])) $nbofusers = $line->qty;
			    if (preg_match('/\sgb\s/i', $prodforline->array_options['options_resource_label'])) $nbofgbs = $line->qty;
			}
		}
	}
